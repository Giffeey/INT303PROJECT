...
public class OrderdetailJpaController implements Serializable {
    ...
    public void create(Orderdetail orderdetail) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        orderdetail.getOrderdetailPK().setIsbn(orderdetail.getBook().getIsbn());
        ...
        try {
            ...
            if (book != null) {
                book.getOrderdetailList().add(orderdetail);
                book = em.merge(book);
            }
            if (orders != null) {
    ...
    public void edit(Orderdetail orderdetail) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            Book bookOld = persistentOrderdetail.getBook();
            Book bookNew = orderdetail.getBook();
            ...
            if (bookOld != null && !bookOld.equals(bookNew)) {
                bookOld.getOrderdetailList().remove(orderdetail);
                bookOld = em.merge(bookOld);
            }
            if (bookNew != null && !bookNew.equals(bookOld)) {
                bookNew.getOrderdetailList().add(orderdetail);
    ...
    public void destroy(OrderdetailPK id) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            Book book = orderdetail.getBook();
            if (book != null) {
                book.getOrderdetailList().remove(orderdetail);
                book = em.merge(book);
            }
            Orders orders = orderdetail.getOrders();
            if (orders != null) {
